...
import { decorations, BADGE } from './Journal.tsx'
...
/**
 * The lane graph, for the log window only.
 *
 * Every row is an SVG with a handful of paths — 841 elements for 161 commits —
 * so it belongs somewhere opened on purpose, not in a pane that re-renders
 * whenever a splitter moves.
 */
export function Graph({
  ...
  // open on the commit the window is about, not at the top of history
  useEffect(() => here.current?.scrollIntoView({ block: 'center' }), [sel, commits])
        ...
        return (
          <div
            ...
            className={`flex cursor-default items-center gap-2 pr-2 ${
              on ? 'bg-sel' : 'hover:bg-panel'
            }`}
          >
            <svg width={railW} height={ROW} className="shrink-0" aria-hidden>
              {incoming
                // a lane curving into this commit is drawn by its up-edge instead
            ...
            <span className={`min-w-0 flex-1 truncate ${c.parents.length > 1 ? 'text-muted' : ''}`}>
              {c.subject}
            </span>
