...
handle('git:discard', (cwd: string, tracked: string[], untracked: string[], toHead: boolean) =>
  G.discard(cwd, tracked, untracked, toHead)
)
handle('git:stashList', (cwd: string) => G.stashList(cwd))
handle('git:stashSave', (cwd: string, msg: string, paths: string[]) =>
  G.stashSave(cwd, msg, paths)
)
...
handle('git:commitAt', (cwd: string, ref: string) => G.commitAt(cwd, ref))
handle('git:renameStash', (cwd: string, ref: string, msg: string) => G.renameStash(cwd, ref, msg))
